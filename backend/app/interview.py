...
def start_session(candidate):
    seeds = [
        x for x in SEEDS
        if x[0] in candidate["completed_days"]
    ]
...
def submit_answer(session, text):
    ...
    if sc < 70:
        q = (
            "What specific failure mode or trade-off "
            "would you watch for in production?"
        )

    elif sc < 82:
        q = (
            "Give a concrete example and explain why "
            "you would choose that approach."
        )

    else:
        q = (
            "How would you evaluate and monitor this "
